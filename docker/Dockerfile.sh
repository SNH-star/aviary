...
FROM scratch
COPY --from=0 / /
# WORKDIR is set to / (not /aviary) here to avoid pixi resolving the project root
# ambiguously against the /aviary directory copied in from stage 0, which was
# previously causing the shell-hook path to double up (/aviary/aviary/.pixi/)
WORKDIR /
# Add Pixi to PATH
ENV PATH="/root/.pixi/bin:${PATH}"
ENV PYTHONNOUSERSITE=1
ENV SINGLEM_METAPACKAGE_PATH=/db/singlem \
    ...
    CHECKM_DATA_PATH=/db/checkm/2015_01_16_v2 \
    TMPDIR=/tmp
# cd into /aviary explicitly for the shell-hook call so the project root resolves correctly
RUN cd /aviary && pixi shell-hook --frozen --no-install | grep -v set_env_vars.sh >> /run_via_pixi.profile
