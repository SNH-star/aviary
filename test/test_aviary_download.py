...
# Persistent location for downloaded databases + checkpoints. Downloads land
# here (NOT in pytest's ephemeral tmp_path) so a run killed by OOM/walltime can
# be restarted and resume where it left off. Point this at a roomy disk (e.g.
# /scratch) -- GTDB alone is ~100 GB.
...
def test_configure_download_installs_database(db, env_var, artifact_globs, monkeypatch):
    if os.path.isfile(_verified_marker(db)):
        pytest.skip(
...
# --------------------------------------------------------------------------- #
@pytest.mark.download
def test_download_then_complete_walkthrough(tmp_path, monkeypatch):
