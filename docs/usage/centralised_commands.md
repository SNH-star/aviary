...
The `--short-read-mapper`/`--long-read-mapper` flag family, shared by
`recover`, `assemble`, `complete` and `isolate`, is documented separately
under [Read mappers](mappers.md).
